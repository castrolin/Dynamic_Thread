...
IMPLEMENT_DYNAMIC(CRecipeSettingDlg, CDialog)

	CRecipeSettingDlg::CRecipeSettingDlg(CWnd* pParent, CString RecipePATH, CGetRecipe *pSystemRecipe)
	: CDialog(CRecipeSettingDlg::IDD, pParent), m_pRecipe(RecipePATH)
{
	...
	this->Create(IDD_DIALOG_OPTION_RECIPE, pParent);
...
BEGIN_MESSAGE_MAP(CRecipeSettingDlg, CDialog)
	ON_WM_SHOWWINDOW()
	...
	ON_NOTIFY(NM_CLICK, IDC_LIST_RECIPE_INFO, OnNMClickGrid_Recipe)
	ON_NOTIFY(NM_DBLCLK, IDC_LIST_RECIPE_INFO, OnNMDbClickGrid_Recipe)

	ON_BN_CLICKED(IDC_BUTTON_SYSTEM_RECIPE_RELOAD, &CRecipeSettingDlg::OnBnClickedButtonSystemRecipeReload)
	ON_BN_CLICKED(IDC_BUTTON_SYSTEM_RECIPE_SAVE, &CRecipeSettingDlg::OnBnClickedButtonSystemRecipeSave)
...
BOOL CRecipeSettingDlg::OnInitDialog()
{
	...
	m_ParamList.InsertColumn(2,	_T("Value"),	LVCFMT_CENTER, 135, 2);
	...
	m_RecipeList.InsertColumn(2,	_T("RecipeName"),	LVCFMT_LEFT, 180, 2);
	m_RecipeList.InsertColumn(3,	_T("FileName"),	LVCFMT_LEFT, 180, 2);
...
void CRecipeSettingDlg::OnShowWindow( BOOL bShow, UINT nStatus )
{
	...
	if(bShow == TRUE)
	{
		...
		GetDlgItem(IDC_STATIC_SYSTEM_CURRENT_RECIPE)->SetWindowText(FormatString("Current Rcipe: %s", m_pSystemRecipe->GetRecipeName()));
...
long CRecipeSettingDlg::OnMessageChangeLang( WPARAM wparam, LPARAM lparam )
{
	CString DlgName = "RecipeSettingDlg";
...
void CRecipeSettingDlg::OnNMClickGrid(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CRecipeSettingDlg::OnNMDbClickGrid( NMHDR* pNMHDR, LRESULT* pResult )
{
...
void CRecipeSettingDlg::OnNMClickGrid_Recipe(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMITEMACTIVATE pNMItemActivate = reinterpret_cast<LPNMITEMACTIVATE>(pNMHDR);
...
void CRecipeSettingDlg::OnNMDbClickGrid_Recipe( NMHDR* pNMHDR, LRESULT* pResult )
{
	LPNMITEMACTIVATE pNMItemActivate = reinterpret_cast<LPNMITEMACTIVATE>(pNMHDR);
	...
	if(m_RecipeList.SubItemHitTest(&info) != -1)
	{

		m_Recipe_Row = info.iItem;
		m_Recipe_Col = info.iSubItem;

		CString nItemName = m_RecipeList.GetItemText(m_Recipe_Row, 2);

		if(m_pRecipe.LoadRecipe(nItemName) == false)
			AfxMessageBox("Load Recipe Process Fail");
		else
		{
			ShowRecipeData();

			AfxMessageBox("Load Pattern Process Success");
		}

	}
}

void CRecipeSettingDlg::UpdateList()
{
...
void CRecipeSettingDlg::OnBnClickedButtonSystemRecipeReload()
{
...
void CRecipeSettingDlg::OnBnClickedButtonSystemRecipeSave()
{
	...
	ShowSystemMessageBox("系統", "參數儲存完成", true);
}

void CRecipeSettingDlg::ShowRecipeData()
{
...
bool CRecipeSettingDlg::ProcessRecipeID()
{
	for(int x = 0; x < m_RecipeList.GetItemCount(); x ++)
	{
		CGetRecipe TempRecipe(Project_Path);

		CString RecipeName = m_RecipeList.GetItemText(x, 1);
		if(TempRecipe.LoadRecipe(RecipeName) == false)
		{
			ShowSystemMessageBox("系統", "讀取參數異常", true);
			return false;
		}

		TempRecipe.Recipe_Data.st_RecipeInfo.nRcipeID = x + 1;

		TempRecipe.SaveRecipe();
...
void CRecipeSettingDlg::CopyPatternOnForAll( CString SourcePattern)
{
	CDiskObject DiskObject;
